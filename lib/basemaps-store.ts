...
async function ensureTable(): Promise<void> {
  ...
  await getPool().query(`
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = '_postgis_frontend_basemaps')
         AND NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = '_pgmaps_basemaps')
      THEN
        ALTER TABLE _postgis_frontend_basemaps RENAME TO _pgmaps_basemaps;
      END IF;
    END $$
  `);
  await getPool().query(`
    CREATE TABLE IF NOT EXISTS _pgmaps_basemaps (
      id         TEXT PRIMARY KEY,
      name       TEXT NOT NULL,
  ...
  for (const b of SEED_BASEMAPS) {
    await getPool().query(
      `INSERT INTO _pgmaps_basemaps (id, name, style_url)
...
export async function listBasemaps(): Promise<BasemapEntry[]> {
  ...
  const { rows } = await getPool().query(
    `SELECT id, name, style_url, created_at FROM _pgmaps_basemaps ORDER BY created_at ASC`
  );
  return rows.map((r) => ({
    id: r.id,
    name: r.name,
...
export async function addBasemap(name: string, styleUrl: string, id?: string): Promise<BasemapEntry> {
  ...
  const { rows } = await getPool().query(
    `INSERT INTO _pgmaps_basemaps (id, name, style_url) VALUES ($1, $2, $3) RETURNING created_at`,
...
export async function updateBasemap(id: string, name: string, styleUrl: string): Promise<void> {
  ...
  const { rowCount } = await getPool().query(
    `UPDATE _pgmaps_basemaps SET name = $2, style_url = $3 WHERE id = $1`,
...
export async function deleteBasemap(id: string): Promise<void> {
  await ensureTable();
  await getPool().query(`DELETE FROM _pgmaps_basemaps WHERE id = $1`, [id]);
}
