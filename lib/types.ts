...
export interface LayerLabel {
  field: string;      // column name OR template like "{name} ({pop})"
  size: number;
  color: string;
  haloColor: string;
  haloWidth: number;
...
export function migrateLayerControls(raw: any): LayerControl[] {
  ...
  for (const f of (raw.controls ?? raw.filters ?? [])) {
    ...
    if (f.type === "fill") {
      ...
      controls.push({ ...base, type: "categorical", column: f.column ?? "", rules: migratedRules, defaultColor: f.defaultColor ?? cfStyle?.defaultColor ?? "#aaaaaa", hiddenValues: f.hiddenValues ?? [], target: f.target ?? "fill", label: f.label });
      ...
      controls.push({ ...base, type: "threshold", column: f.column ?? "", threshold: f.threshold ?? 0, aboveColor: f.aboveColor ?? "#22c55e", belowColor: f.belowColor ?? "#ef4444", target: f.target ?? "fill", label: f.label, ...(f.ranges ? { ranges: f.ranges, defaultColor: f.defaultColor } : {}) });
