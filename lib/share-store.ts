...
async function ensureTable(): Promise<void> {
  ...
  `);
  await getPool().query(`ALTER TABLE _pgmaps_maps ADD COLUMN IF NOT EXISTS is_public BOOLEAN NOT NULL DEFAULT FALSE`);
  await getPool().query(`ALTER TABLE _pgmaps_maps ADD COLUMN IF NOT EXISTS archived BOOLEAN NOT NULL DEFAULT FALSE`);
  await getPool().query(`ALTER TABLE _pgmaps_maps ADD COLUMN IF NOT EXISTS password_hash TEXT`);
  await getPool().query(`ALTER TABLE _pgmaps_maps ADD COLUMN IF NOT EXISTS expires_at TIMESTAMPTZ`);
  _ready = true;
}
...
export async function getShare(id: string, password?: string): Promise<ShareGetResult> {
  ...
  const { rows } = await getPool().query(
    `SELECT state_json, connection_id, password_hash, expires_at
     FROM _pgmaps_maps WHERE id = $1 AND is_public = TRUE`,
    [id]
  );
...
export async function deleteShare(id: string): Promise<void> {
  await ensureTable();
  await getPool().query(
    `UPDATE _pgmaps_maps SET is_public = FALSE WHERE id = $1`,
    [id]
  );
}
...
export async function listShares(): Promise<ViewIndexEntry[]> {
  await ensureTable();
  const { rows } = await getPool().query(
    `SELECT id, name, created_at, updated_at FROM _pgmaps_maps WHERE is_public = TRUE ORDER BY created_at DESC`
  );
  return rows.map((r) => ({
    id: r.id,
    name: r.name,
