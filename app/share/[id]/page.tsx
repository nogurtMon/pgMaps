...
export default function ShareViewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function applyConfig(config: any) {
    ...
    const startView = config.homeView ?? config.view;
    if (startView) { setInitialView(startView); setMapZoom(startView.zoom); }
    ...
    return (
      <div className="h-screen flex items-center justify-center">
        <div className="text-center space-y-3 max-w-xs">
          ...
          <a href="/" className="text-xs text-muted-foreground underline underline-offset-2">Go to pgMaps</a>
        </div>
      </div>
    );
  }
    ...
    <div className="h-[100dvh] overflow-hidden flex">
      <div className="relative flex-1 min-w-0 h-full">
        <MaplibreMap
          ...
          hideZoom
          shareControls
          onViewChange={(v) => { setMapBounds(v?.bounds); if (v?.zoom !== undefined) setMapZoom(v.zoom); }}
        />
