...
export default function Home() {
  ...
  const [homeView, setHomeView] = React.useState<{ longitude: number; latitude: number; zoom: number } | undefined>(undefined);
  ...
  // Read ?name= and ?folder= for new blank maps created via the dialog
  const pendingFolderIdRef = React.useRef<string | null>(null);
  React.useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const nameParam = params.get("name");
    const folderParam = params.get("folder");
    if (nameParam) setActiveViewName(nameParam);
    if (folderParam) {
      pendingFolderIdRef.current = folderParam;
      setFolderId(folderParam);
      fetch("/api/pg/folders")
        .then(r => r.json())
        .then(d => {
          const f = (d.folders ?? []).find((x: any) => x.id === folderParam);
  ...
  React.useEffect(() => {
    ...
    fetch(`/api/pg/saved-views/${viewId}`)
      ...
      .then((data) => {
        ...
        if (v.state_json.homeView) setHomeView(v.state_json.homeView);
        const startView = v.state_json.homeView ?? v.state_json.view;
        if (startView) {
          setZoomTarget({ center: [startView.longitude, startView.latitude], zoom: startView.zoom });
        }
        setActiveViewName(v.name ?? null);
        setActiveViewId(viewId);
        if (v.is_public) setShareId(viewId);
        setIsTemplate(!!v.is_template);
        if (v.folder_id) {
          setFolderId(v.folder_id);
          fetch("/api/pg/folders")
            .then(r => r.json())
            .then(d => {
              const f = (d.folders ?? []).find((x: any) => x.id === v.folder_id);
              if (f) setFolderName(f.name);
            })
            .catch(() => {});
        }
  ...
  const shareIdRef = React.useRef<string | null>(null);
  const creatingRef = React.useRef(false);
  React.useEffect(() => { activeViewIdRef.current = activeViewId; }, [activeViewId]);
  React.useEffect(() => { shareIdRef.current = shareId; }, [shareId]);
  const mapViewRef = React.useRef(mapView);
  React.useEffect(() => { mapViewRef.current = mapView; }, [mapView]);
  const homeViewRef = React.useRef(homeView);
  React.useEffect(() => { homeViewRef.current = homeView; }, [homeView]);
  const activeViewNameRef = React.useRef(activeViewName);
  React.useEffect(() => { activeViewNameRef.current = activeViewName; }, [activeViewName]);
  ...
  async function saveAsTemplate() {
    ...
    if (!name || !activeViewIdRef.current || !connectionId) return;
    ...
    try {
      ...
      const res = await fetch("/api/pg/saved-views", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ connectionId, id: newId, name, state: buildState() }),
      });
  ...
  React.useEffect(() => {
    ...
    if (!currentId) {
      ...
      fetch("/api/pg/saved-views", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ connectionId, id, name: activeViewNameRef.current || "Untitled map", state: buildState(), folder_id: pendingFolderIdRef.current }),
      }).then(res => {
        if (res.ok) {
          setActiveViewId(id);
    ...
    const t = setTimeout(async () => {
      ...
      // Save the view first; then sync the share sequentially so they never
      // race on the same DB row (activeViewId === shareId for public views).
      await fetch(`/api/pg/saved-views/${currentId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ state }),
      }).catch(() => {});

      // Keep the share in sync if one exists — so layer changes (style, zoom
      // range, labels, filters) are reflected without the user re-sharing.
      const sid = shareIdRef.current;
      if (sid && state.layers.length > 0) {
        try {
          const res = await fetch(`/api/share/${sid}`, {
            ...
            body: JSON.stringify({
              ...
              name: activeViewNameRef.current ?? undefined,
          ...
          if (!res.ok) console.error("[share-sync] PUT failed:", res.status, await res.text().catch(() => ""));
        } catch (e) {
          console.error("[share-sync] network error:", e);
        }
      }
    }, 1500);
    return () => clearTimeout(t);
  }, [layers, basemap, connectionId, loaded, homeView, markdown]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    ...
    <div className="h-screen overflow-hidden grid grid-rows-[auto_1fr]">
      <header className="bg-background border-b px-3 py-1.5 flex items-center gap-3 shrink-0">
          ...
          <DropdownMenuContent align="start" className="w-52">
            ...
            {activeViewIdRef.current && (
              <DropdownMenuItem
                ...
                onClick={() => {
                  setTemplateNameInput(activeViewNameRef.current ?? "");
        ...
        <div className="flex items-center gap-1 flex-1 min-w-0">
          ...
          {editingName ? (
            ...
            >
              {activeViewName
                ? <span className="text-sm font-semibold truncate block group-hover:text-primary transition-colors">{activeViewName}</span>
                : <span className="text-sm font-semibold text-muted-foreground block group-hover:text-foreground transition-colors">Untitled map</span>
              }
            </button>
          )}
        </div>
      ...
      <Dialog open={saveAsTemplateOpen} onOpenChange={(open) => { if (!open) setSaveAsTemplateOpen(false); }}>
        <DialogContent className="sm:max-w-sm">
          ...
          <Input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "Escape") setSaveAsTemplateOpen(false);
            }}
          />
          <DialogFooter>
