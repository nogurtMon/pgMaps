...
export async function POST(req: NextRequest) {
  const { connectionId, dsn: legacyDsn, schema, table, rows, srid } = await req.json();
  ...
  const hasGeom = rows[0]?.geomJson != null || rows[0]?.wkbHex != null;
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i += SUB_BATCH) {
      ...
      for (const row of slice) {
        ...
        if (hasGeom) {
          if (row.wkbHex) {
            params.push(row.wkbHex);
            placeholders.push(`ST_GeomFromEWKB(decode($${params.length}, 'hex'))`);
          } else {
