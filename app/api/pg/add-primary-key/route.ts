...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Add id column as SERIAL (auto-fill existing rows), then promote to PK
    await client.query(
      `ALTER TABLE ${ident(schema, table)} ADD COLUMN id SERIAL`
    );
    await client.query(
      `ALTER TABLE ${ident(schema, table)} ADD PRIMARY KEY (id)`
    );
