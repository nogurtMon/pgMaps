...
export async function POST(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // Prefer objectId-based pagination — works on all ArcGIS servers regardless of WHERE clause
      // or resultOffset support. Falls back to resultOffset if returnIdsOnly isn't available.
      let allObjectIds: number[] | null = null;
      let total = 0;
      try {
        const idsRes = await fetch(
          `${layerUrl}/query?where=${encodeURIComponent(whereClause)}&returnIdsOnly=true&f=json`,
          { headers: { Accept: "application/json" } }
        );
        if (idsRes.ok) {
          const idsJson = await idsRes.json();
          if (Array.isArray(idsJson.objectIds)) {
            allObjectIds = idsJson.objectIds as number[];
            total = (allObjectIds as number[]).length;
          }
        }
      } catch {}

      // Fall back to count-only if returnIdsOnly failed
      if (!allObjectIds) {
        try {
      ...
      async function fetchBatch(offset: number): Promise<any[]> {
        ...
        while (true) {
          let url: string;
          if (allObjectIds) {
            const chunk = allObjectIds.slice(offset, offset + size);
            if (chunk.length === 0) return [];
            url = `${layerUrl}/query?objectIds=${chunk.join(",")}&outFields=${outFields}&geometryPrecision=6&f=geojson`;
          } else {
            url = `${layerUrl}/query?where=${encodeURIComponent(whereClause)}&outFields=${outFields}&resultOffset=${offset}&resultRecordCount=${size}&geometryPrecision=6&f=geojson`;
          }
          ...
          if (res.status === 504 || res.status === 503 || res.status === 502 || res.status === 500) {
            ...
            if (smaller < 10) throw new Error(`ArcGIS returned HTTP ${res.status} even at ${size} records per request. The service may be unavailable.`);
            size = smaller;
      ...
      try {
        ...
        // Suppress unhandled rejections from other in-flight prefetch promises
