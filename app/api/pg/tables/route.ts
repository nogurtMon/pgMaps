...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // geometry_columns doesn't exist → PostGIS not enabled. Not a connection error —
    // return empty table list with a flag so the UI can show an install prompt.
    if (/geometry_columns/i.test(e.message)) {
      return NextResponse.json({ tables: [], postgisRequired: true });
    }
