...
import type { TableRow, MapLayer, LayerControl, LayerLabel, AttrOperator, FillColorRule, TemporalMode, ColorRange, ShapeRule } from "@/lib/types";
import { GEOMETRIC_ICONS, ICON_CATEGORIES, findIcon, iconDataUri } from "@/lib/point-icons";
...
function LayerFilterEditor({
  ...
  const [zoomRangeOpen, setZoomRangeOpen] = React.useState(false);
      ...
      {activeTab === "style" && (<>
        ...
        <div className="border border-border/50 rounded-lg overflow-hidden">
          <div
            role="button" tabIndex={0}
            className="flex items-center gap-1.5 w-full px-2.5 py-1.5 bg-muted/20 hover:bg-muted/40 transition-colors select-none cursor-pointer"
            onClick={() => setZoomRangeOpen(o => !o)}
            onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setZoomRangeOpen(o => !o); } }}
          >
            <ChevronDown className={`h-3 w-3 text-muted-foreground/60 transition-transform shrink-0 ${zoomRangeOpen ? "" : "-rotate-90"}`} />
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide flex-1 text-left">Zoom Range</span>
            <span className="text-[10px] text-muted-foreground">
              {layer.minZoom || layer.maxZoom ? `${layer.minZoom ?? 0}–${layer.maxZoom ?? 20}` : "Always"}
            </span>
          </div>
          {zoomRangeOpen && (
            <div className="px-2.5 py-2 space-y-2">
              <p className="text-[10px] text-muted-foreground">Hide this layer outside this zoom range. Useful for large datasets that are slow to render at low zoom, or detail layers you only want up close.</p>
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-muted-foreground w-8 shrink-0">Min</span>
                <Slider
                  min={0} max={20} step={1}
                  value={[layer.minZoom ?? 0]}
                  onValueChange={([v]) => onUpdateLayer(layer.id, { minZoom: v === 0 ? undefined : v })}
                  className="flex-1"
                />
                <span className="text-xs text-muted-foreground w-16 text-right shrink-0">
                  {layer.minZoom ? `≥ z${layer.minZoom}` : "Any"}
                </span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Slider
                  ...
                  onValueChange={([v]) => onUpdateLayer(layer.id, { maxZoom: v === 20 ? undefined : v })}
                  className="flex-1"
                />
                <span className="text-xs text-muted-foreground w-16 text-right shrink-0">
                  {layer.maxZoom ? `≤ z${layer.maxZoom}` : "Any"}
                </span>
              </div>
            </div>
          )}
        ...
        <div className="border border-border/50 rounded-lg overflow-hidden">
          <div
            ...
            onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setLabelOpen(o => !o); } }}
          >
            <ChevronDown className={`h-3 w-3 text-muted-foreground/60 transition-transform shrink-0 ${labelOpen ? "" : "-rotate-90"}`} />
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide flex-1 text-left">Labels</span>
            {layer.label?.field && (
              <span className="text-[10px] text-primary truncate max-w-[80px]">{layer.label.field}</span>
            )}
          </div>
          {labelOpen && (
            <LabelEditor layer={layer} nonGeomCols={nonGeomCols} onUpdateLayer={onUpdateLayer} />
          )}
        </div>
...
// ─── label editor ─────────────────────────────────────────────────────────────
const DEFAULT_LABEL: LayerLabel = { field: "", size: 12, color: "#333333", haloColor: "#ffffff", haloWidth: 1.5, minZoom: 0 };

function LabelEditor({ layer, nonGeomCols, onUpdateLayer }: {
  layer: MapLayer;
  nonGeomCols: { name: string; dataType: string; isGeom: boolean }[];
  onUpdateLayer: (id: string, patch: Partial<MapLayer>) => void;
}) {
  const lbl = layer.label ?? null;
  const enabled = !!lbl?.field;

  function set(patch: Partial<LayerLabel>) {
    const next: LayerLabel = { ...DEFAULT_LABEL, ...(lbl ?? {}), ...patch };
    onUpdateLayer(layer.id, { label: next });
  }

  function disable() {
    onUpdateLayer(layer.id, { label: null });
  }

  const fieldRef = React.useRef<HTMLTextAreaElement>(null);

  function insertAtCursor(text: string) {
    const current = lbl?.field ?? "";
    const el = fieldRef.current;
    if (!el) { set({ field: current + text }); return; }
  ...
  return (
    <div className="px-2.5 py-2 space-y-2.5">
      {/* Enable toggle */}
      <div className="flex items-center gap-2">
        <span className="text-[10px] text-muted-foreground flex-1">Show labels on map</span>
        <button
          onClick={() => enabled ? disable() : set({ field: nonGeomCols[0]?.name ?? "" })}
          className={`relative inline-flex h-4 w-7 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors focus:outline-none ${enabled ? "bg-primary" : "bg-muted-foreground/30"}`}
        >
          <span className={`inline-block h-3 w-3 rounded-full bg-white shadow transition-transform ${enabled ? "translate-x-3" : "translate-x-0"}`} />
        </button>
      </div>
      ...
      {enabled && lbl && (<>
        {/* Field / template */}
        <div className="space-y-1">
          <span className="text-[10px] text-muted-foreground">Label field or template</span>
          <div className="flex gap-1">
            <textarea
              ref={fieldRef}
              value={lbl.field}
              onChange={e => set({ field: e.target.value })}
              placeholder="e.g. name  or  {name}&#10;{count}"
              rows={2}
              className="flex-1 min-w-0 px-2 py-1 text-xs rounded border bg-background focus:outline-none focus:ring-1 focus:ring-ring resize-y"
            />
            <div className="flex flex-col gap-1 shrink-0">
              <Popover>
                <PopoverTrigger asChild>
                  <button className="h-6 px-2 text-[10px] border rounded bg-muted/40 hover:bg-muted/70 text-muted-foreground">cols</button>
                </PopoverTrigger>
                <PopoverContent className="w-44 p-1" align="end">
                  <p className="text-[10px] text-muted-foreground px-1 pb-1">Click to insert</p>
                  <div className="max-h-40 overflow-y-auto">
                    {nonGeomCols.map(c => (
                      <button key={c.name} onClick={() => insertCol(c.name)}
                        className="w-full text-left px-2 py-0.5 text-xs rounded hover:bg-muted/60 truncate">
                        {c.name}
                      </button>
                    ))}
                  </div>
              ...
              <button
                ...
                className="h-6 px-2 text-[10px] border rounded bg-muted/40 hover:bg-muted/70 text-muted-foreground"
              >
                ↵
              </button>
            </div>
          </div>
          <p className="text-[10px] text-muted-foreground/70">Use column name directly, or template: <code className="font-mono">{"{name}"} km²</code>. Press Enter or the ↵ button for a line break.</p>
        </div>

        {/* Size */}
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-muted-foreground w-10 shrink-0">Size</span>
          <Slider min={8} max={32} step={1} value={[lbl.size]} onValueChange={([v]) => set({ size: v })} className="flex-1" />
          <span className="text-[10px] text-muted-foreground w-6 text-right shrink-0">{lbl.size}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer flex-1">
            <span className="w-5 h-5 rounded border border-border shrink-0 block" style={{ background: lbl.color }} />
            <input type="color" value={lbl.color} onChange={e => set({ color: e.target.value })} className="sr-only" />
            <span className="text-[10px] text-muted-foreground font-mono">{lbl.color}</span>
          </label>
        </div>

        {/* Halo color + width */}
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-muted-foreground w-10 shrink-0">Halo</span>
          <label className="flex items-center gap-1.5 cursor-pointer">
            <span className="w-5 h-5 rounded border border-border shrink-0 block" style={{ background: lbl.haloColor }} />
            <input type="color" value={lbl.haloColor} onChange={e => set({ haloColor: e.target.value })} className="sr-only" />
            <span className="text-[10px] text-muted-foreground font-mono">{lbl.haloColor}</span>
          </label>
          <Slider min={0} max={4} step={0.5} value={[lbl.haloWidth]} onValueChange={([v]) => set({ haloWidth: v })} className="flex-1" />
          <span className="text-[10px] text-muted-foreground w-6 text-right shrink-0">{lbl.haloWidth}</span>
        </div>

        {/* Min zoom */}
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-muted-foreground w-10 shrink-0">From</span>
          <Slider min={0} max={20} step={1} value={[lbl.minZoom]} onValueChange={([v]) => set({ minZoom: v })} className="flex-1" />
          <span className="text-[10px] text-muted-foreground w-12 text-right shrink-0">{lbl.minZoom === 0 ? "Any zoom" : `≥ z${lbl.minZoom}`}</span>
        </div>
      </>)}
    </div>
  );
...
function ConnectionBrowserNode({
  ...
  const [reprojectingTable, setReprojectingTable] = React.useState<string | null>(null);
  const [reprojectLoading, setReprojectLoading] = React.useState(false);
  const [reprojectError, setReprojectError] = React.useState<string | null>(null);
            ...
            return (
              <div key={schema}>
                ...
                {!isCollapsed && schemaTables.map((t) => {
                  ...
                  const isReprojecting = reprojectingTable === key;
                    ...
                    <div key={key} className={`border-b ${alreadyAdded ? "bg-primary/5" : ""}`}>
                      ...
                      >
                        ...
                        <div className="flex flex-col min-w-0 flex-1">
                          <span className="text-xs truncate" title={t.table_name}>{t.table_name}</span>
                          {t.description && (
                            <span className="text-[10px] text-muted-foreground truncate leading-tight">{t.description}</span>
                          )}
                        </div>
                      ...
                      {isReprojecting && (
                        <div className="px-3 pb-2 space-y-1.5 bg-orange-50/50 dark:bg-orange-950/20 border-t">
                          <p className="text-[10px] text-muted-foreground pt-1.5">
                            Rewrites every geometry value to <span className="font-mono">EPSG:4326</span> using <span className="font-mono">ST_Transform</span>. The source SRID is detected automatically. The spatial index will be dropped and should be recreated afterward.
                          </p>
                          <div className="flex gap-1.5 items-center">
                            <Button size="sm" className="h-7 text-xs" disabled={reprojectLoading} onClick={async () => {
                              setReprojectLoading(true); setReprojectError(null);
                              try {
                                const res = await fetch("/api/pg/reproject", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ connectionId: connId, schema: t.table_schema, table: t.table_name, geomCol: t.geom_col }) });
                                const data = await res.json();
                                if (!res.ok) throw new Error(data.error);
                                setReprojectingTable(null); setRefreshKey((k) => k + 1);
                              } catch (e: any) { setReprojectError(e.message); } finally { setReprojectLoading(false); }
                            }}>
                              {reprojectLoading ? "Reprojecting…" : "Reproject to EPSG:4326"}
                            </Button>
                            <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setReprojectingTable(null)}>Cancel</Button>
                          </div>
                          {reprojectError && <p className="text-[10px] text-destructive break-words">{reprojectError}</p>}
                        </div>
                      )}
                    </div>
                  );
          ...
          {contextMenu.target.type === "table" && (() => {
            ...
            return (
              <>
                ...
                <div className="border-t my-1" />
                <button className="w-full text-left px-3 py-1.5 hover:bg-muted" onClick={() => {
                  const key = `${connId}|${t.table_schema}.${t.table_name}`;
                  setReprojectingTable(key);
