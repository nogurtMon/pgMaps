...
// ─── swatch components ────────────────────────────────────────────────────────
function GeomSwatch({ kind, color, strokeColor }: {
...
function LayerEntry({ layer, onToggleVisible }: {
  ...
  const hasDetail = !!(cat || thresh || hasRadiusCtrl || hasLineWidthCtrl);
  ...
  const catLabel = cat
    ? (cat.label || cat.column.replace(/_/g, " "))
    : null;
  const thrLabel = thresh
    ? (thresh.label || thresh.column.replace(/_/g, " "))
    : null;
  ...
  return (
    <div className={`transition-opacity ${!layer.visible ? "opacity-40" : ""}`}>
      ...
      >
        {!cat && (
          <GeomSwatch kind={kind} color={swatchColor} strokeColor={strokeColor} />
        )}
      ...
      {hasDetail && open && (
        <div className="pl-1 mb-1 space-y-1.5">
          {cat && (
            <div>
              {catLabel && <p className="text-[10px] text-muted-foreground mb-0.5 capitalize">{catLabel}</p>}
              <CategoricalLegend ctrl={cat} />
            </div>
          )}
          {thresh && (
            <div>
              {thrLabel && <p className="text-[10px] text-muted-foreground mb-0.5 capitalize">{thrLabel}</p>}
