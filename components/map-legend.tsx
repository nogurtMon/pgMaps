...
// ─── single layer entry ───────────────────────────────────────────────────────
function LayerEntry({ layer, onToggleVisible, zoom }: {
  layer: MapLayer;
  ...
  const hiddenByZoom = !!(zoom !== undefined && ((layer.minZoom && zoom < layer.minZoom) || (layer.maxZoom && zoom > layer.maxZoom)));
  ...
  return (
    <div className={`transition-opacity ${!layer.visible ? "opacity-40" : ""}`}>
      ...
      {(layer.minZoom || layer.maxZoom) && (
        <p className={`text-[9px] mb-0.5 ${hiddenByZoom ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground"}`}>
          {hiddenByZoom
            ? (layer.minZoom && zoom !== undefined && zoom < layer.minZoom
                ? `Hidden — zoom in to ${layer.minZoom}`
                : `Hidden — zoom out to ${layer.maxZoom}`)
            : `Visible at zoom ${layer.minZoom ?? 0}–${layer.maxZoom ?? 20}`}
        </p>
      )}
    ...
    <div className="absolute bottom-6 left-3 z-10 w-64 bg-background/95 backdrop-blur-sm border rounded-lg shadow-lg overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {zoom !== undefined && (
            <span className="inline-flex items-center gap-1 px-2 h-5 rounded-full border border-primary/30 bg-primary/15 text-[11px] font-bold tabular-nums text-primary tracking-tight">
              <span className="leading-none font-medium opacity-70">Zoom:</span>
              <span className="leading-none">{zoom.toFixed(1)}</span>
            </span>
          )}
